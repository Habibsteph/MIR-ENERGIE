export const homeServices = [
  {
    id: "lng-supply", slug: "lng-supply-trading", headline: "Secure supply. Structured around your market.", scope: ["Supplier engagement and international sourcing", "Commercial structuring and supply discussions", "Documentation coordination and procurement strategy"], audience: "Governments, utilities, mining companies and industrial clients.", inputs: "Demand profile, expected volumes, supply duration and delivery requirements.", number: "01", title: "LNG Supply & Trading", tab: "LNG Supply", subtitle: "Global sourcing",
    summary: "Global sourcing, commercial structuring and long-term LNG supply solutions designed around each market’s demand profile.",
    description: "MIR ENERGY sources and structures LNG supply solutions from international markets for governments, utilities, mining companies and industrial clients. Capabilities include supplier engagement, commercial structuring, long-term supply discussions, documentation coordination and market-linked procurement strategies.",
    image: "/import-ship.jpg", alt: "International maritime shipping and energy logistics",
  },
  {
    id: "logistics", slug: "lng-logistics", headline: "Connect every stage of the journey.", scope: ["Shipping coordination and scheduling", "Receiving requirements and terminal interfaces", "Operational documentation and delivery coordination"], audience: "Energy suppliers, utilities, terminal partners and industrial clients.", inputs: "Origin, destination, delivery schedule and receiving requirements.", number: "02", title: "LNG Logistics", detailTitle: "Marine & LNG Logistics", tab: "Logistics", subtitle: "Source to destination",
    summary: "End-to-end coordination of shipping, receiving, transportation and operational interfaces from source to destination.",
    description: "We coordinate the interfaces required to move LNG safely and efficiently from source to destination, including shipping coordination, receiving requirements, terminal interfaces, scheduling and operational documentation.",
    image: "/oil-pumpjacks-desert.png", alt: "Oil pumpjacks in the desert at sunset",
  },
  {
    id: "storage-regasification", slug: "storage-regasification", headline: "Make energy accessible where it is needed.", scope: ["Receiving and storage requirements", "Regasification infrastructure coordination", "Capacity planning and future scalability"], audience: "Utilities, industrial demand centers and energy infrastructure partners.", inputs: "Capacity needs, site constraints, demand profile and future expansion.", number: "03", title: "Storage & Regasification", tab: "Storage & Regasification", subtitle: "Receiving and storage",
    summary: "Development and coordination of LNG receiving, storage and regasification infrastructure adapted to local demand and operating conditions.",
    description: "MIR ENERGY helps develop and coordinate the receiving, storage and regasification infrastructure required to make LNG accessible to local demand centers. Solutions are structured around capacity needs, site constraints, delivery profiles and future scalability.",
    image: "/silos-mir-branded-realistic.png", alt: "Industrial storage infrastructure with MIR branding",
  },
  {
    id: "infrastructure", slug: "energy-infrastructure", headline: "Build the infrastructure that enables growth.", scope: ["Early-stage feasibility and technical coordination", "Project structuring and partner alignment", "Implementation support and infrastructure deployment"], audience: "Governments, industrial operators and strategic infrastructure partners.", inputs: "Project location, feasibility status, technical constraints and implementation timeline.", number: "04", title: "Energy Infrastructure Development", tab: "Infrastructure", subtitle: "Project development",
    summary: "From feasibility and technical coordination to infrastructure deployment, MIR ENERGY helps structure the assets required to bring natural gas closer to industrial and strategic consumers.",
    description: "We support the development of the infrastructure required to connect LNG supply with industrial and strategic users, from early-stage feasibility and technical coordination to project structuring and implementation support.",
    image: "/desert-energy-engineers.png", alt: "Engineers reviewing plans at a desert energy facility",
  },
];

