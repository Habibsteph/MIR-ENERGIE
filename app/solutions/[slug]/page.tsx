...
import { solutions, legacySolutions } from "@/lib/content";
import { pageMetadata } from "@/lib/metadata";

type Props = { params: Promise<{ slug: string }> };
export function generateStaticParams() { return [...solutions.map(s => ({ slug: s.slug })), ...Object.keys(legacySolutions).map(slug => ({ slug }))]; }
export async function generateMetadata({ params }: Props) {
  const { slug } = await params;
  if (Object.hasOwn(legacySolutions, slug)) permanentRedirect(`/solutions/${legacySolutions[slug]}`);
...
export default async function SolutionPage({ params }: Props) {
  ...
  if (!s) notFound();
  return <main id="top">
    <section className={`services-cover${s.id === "gas" ? " storage-solution-cover" : ""}`}><Image src={s.image} alt={s.alt} fill priority sizes="100vw" className="cover-img" /><div className="services-cover-shade" /><SiteHeader overlay />
    ...
    <section className="services-capabilities page-pad"><div className="services-capabilities-intro"><p className="overline">Scope of support</p><h2>Expertise shaped around your project.</h2></div><div className="solution-scope">{s.scope.map((item,i) => <article key={item.title}><span>0{i+1}</span><div><h3>{item.title}</h3><p>{item.copy}</p></div></article>)}</div></section>
